...
const sourceIds = ['bwapi', 'opprimobot', 'bwta2']
...
async function assertRealFile(file) {
...
async function addSources({ buildDirectory, entries, info, lock, root }) {
...
async function addRecipe({ entries, info, root }) {
  ...
  for (const name of opprimoRecipePaths) {
    ...
    if (actual.toString('utf8').replaceAll('\r\n', '\n') !==
...
export async function verifiedBoostFiles(buildDirectory, boost, dependencyLock) {
  ...
  if (
    dependencyLock.schemaVersion !== 1 || dependencyLock.artifacts.length !== 1 ||
    boost?.directory !== boostDirectory || boost.inventory !== boostInventory ||
    ...
    !Array.isArray(boost.files) || boost.files.length < 2 || boost.files.length > 12000
  ) {
  ...
  for (const item of boost.files) {
    ...
    if (totalBytes > boostExpandedLimit) throw new Error('Boost header inventory exceeds size limit')
    const file = path.join(buildDirectory, boostDirectory, ...item.path.split('/'))
    const stat = await lstat(file)
    if (!stat.isFile() || stat.isSymbolicLink() || stat.size !== item.sizeBytes ||
        !samePath(await realpath(file), file)) {
...
export async function packageOpprimobot({
  ...
  const entries = [['bin/OpprimoBot.exe', executable]]
  const lock = await json(path.join(repository, 'source-lock.json'))
  validate('sourceLock', lock)
  const sources = await addSources({ buildDirectory: directory, entries, info, lock, root: repository })
  await addRecipe({ entries, info, root: repository })

  const boostFiles = await verifiedBoostFiles(
    directory,
    info.dependencies?.boost,
    await json(path.join(repository, 'native/dependencies.json')),
  )
  ...
  const notices = [
    ...
    ['OpprimoBot README and citation', 'notices/OpprimoBot-README.md', 'source/opprimobot/README.md'],
    ...
    ['BSD-style (BWTA2 filesystem)', 'notices/BWTA2-FILESYSTEM-LICENSE.txt', 'bots/opprimobot/BWTA2-FILESYSTEM-LICENSE.txt'],
    ...
    ['Attribution, modifications, and source', 'notices/RELEASE.txt', 'bots/opprimobot/RELEASE.txt'],
  ]
  for (const [, target, source] of notices) {
    const bytes = source.startsWith('source/')
      ? entries.find(([name]) => name === source)?.[1]
      : source.startsWith('boost/')
        ? boostFiles.find(([name]) => name === source.slice('boost/'.length))?.[1]
  ...
  entries.push(['source/BUILD.md', await readFile(path.join(repository, 'bots/opprimobot/BUILD.md'))])
  entries.push(['source/build-info.json', Buffer.from(JSON.stringify(info, null, 2) + '\n')])
  for (const dir of [
    'work/', 'work/bwapi-data/', 'work/bwapi-data/AI/',
  ...
  const pkg = {
    ...
    permissions: review
      ? { ...candidate.permissions, localDistribution: { status: 'unreviewed', evidence: 'Review-only archive; publication is not approved.' } }
      : candidate.permissions,
    modifications: [
      {
        modifier: 'ShieldBattery', date: '2026-09-23', scope: 'bot',
        summary: 'Disabled game speed, user input, debug display, chat controls, and the bot timer; the external client exits after one match.',
      },
      {
        modifier: 'ShieldBattery', date: '2026-09-23', scope: 'bot',
        summary: 'Disabled strategy history, result and profiler persistence, and replaced the Pathfinder worker with at most one synchronous path calculation per callback.',
      },
      {
        modifier: 'ShieldBattery', date: '2026-09-23', scope: 'dependency',
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  if (!buildDirectory || !releaseId || rest.length > 1 || (rest.length && rest[0] !== '--review')) {
    throw new Error('Usage: node tools/package-opprimobot.mjs <build-directory> opprimobot-sb-N [--review]')
  }
