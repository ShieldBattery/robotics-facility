...
async function toolchain(javaHome: string | undefined) {
  ...
  } = await inspectJavaToolchain(path.resolve(javaHome))
...
export async function buildMarineHell({
  ...
  const jbwapiArgs = await argumentFile(path.join(compile, 'jbwapi.args'), javaSources)
  const marineArgs = await argumentFile(path.join(compile, 'marine.args'), [marineSource])
  ...
  const entries: [string, Buffer][] = [
    [
      ...
      makeManifest(
        'TestBot1',
        dependencies.map(({ artifact }) => artifact),
      ),
    ],
  ]
  ...
  for (const source of sources) {
    ...
    verified.push({
      ...(await verifyPreparedSource(source, directory)),
      id: source.id,
