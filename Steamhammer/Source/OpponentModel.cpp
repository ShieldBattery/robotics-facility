...
OpeningPlan OpponentModel::predictEnemyPlan() const
{
    ...
    // Data from other maps will be misleading.
...
void OpponentModel::considerOpenings()
{
    ...
    for (const GameRecord * record : _pastGameRecords)
    {
        if (sameMatchup(*record))
        {
			...
			if (record->getOpeningScore() >= 0.0)		// game records missing eval have score=-1
			{
				info.evalGames += 1;
				info.totalEval += record->getOpeningScore();
				
				_summary.totalEvalGames += 1;
				_summary.totalEval += record->getOpeningScore();
			}
	...
	UAB_ASSERT(_summary.totalEvalGames >= 0, "bad total");
    ...
    // This bypasses the map and evaluation weighting below.
    ...
    // Compute "weighted" win rates which combine
	// - overall win rates
	// - win rates on the current map
	// - the opening evaluations
    // The estimate is ad hoc and definitely unsound.
	double overallMeanEval = _summary.getMeanEval();
    for (auto it = _summary.openingInfo.begin(); it != _summary.openingInfo.end(); ++it)
    {
        ...
        // So let's pretend that this adjustment makes sense.
		info.weightedWins = (1.0 + sqrt(info.sameWins)) * info.sameWins + info.otherWins;
		info.weightedGames = (1.0 + sqrt(info.sameGames)) * info.sameGames + info.otherGames;

		// Prefer the best-evaluated openings.
		// It's an attempt to pick openings that look good at first, even if the bot is losing
		// every game--a good start shows promise over a bad start.
		if (_summary.totalEvalGames > 0)
		{
			double diffFromMean = info.getMeanEval() - overallMeanEval;		// positive or negative

			double evalPower = 1.0 + 2 * diffFromMean;						// commonly close to 1.0

			info.weightedWins *= evalPower;

			//UAB_MESSAGE("%s %s diff=%g, evalPower=%g -> wWins=%g wGames=%g",
			//	BWAPI::Broodwar->enemy()->getName().c_str(),
			//	it->first.c_str(), diffFromMean, evalPower, info.weightedWins, info.weightedGames);
		}
	}
...
void OpponentModel::read()
{
    ...
    {
        std::ifstream inFile;
		openReadFile(inFile, _filename);
		if (!inFile.good())
		{
			// No dynamic file in the read directory or static file in the prepared data directory.
			// That's OK, it just means we don't have any experience yet.
			// Or the file can't be read. In that case, we ignore the error. What else can we do?
			return;
		}
...
// Count the game records with the same matchup.
// Used in deciding whether there are enough to draw conclusions.
int OpponentModel::nGamesSameMatchup() const
{
	int count = 0;
	for (const GameRecord * record : getRecords())
	{
		if (sameMatchup(*record))
		{
			++count;
