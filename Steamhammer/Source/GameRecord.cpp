...
// Read an integer that is on a line by itself.
...
// Read a floating point double that is on a line by itself.
double GameRecord::readDouble(std::istream & input)
{
	std::string line;
	double f;
...
// Read a floating point double from a string.
double GameRecord::readDouble(std::string & s)
{
	std::istringstream lineStream(s);
	double f;
	if (lineStream >> f)
	...
	BWAPI::Broodwar->printf("read bad double");
	throw game_record_read_error();
}
...
// Read a 3.0  or 4.2.2 game record.
...
// opening
// IF 4.2.2, a line with the evaluation when reaching the end of the opening book
...
void GameRecord::read_v3_or_4(std::istream & input, bool includeOpeningScore)
{
	...
	if (includeOpeningScore)
	{
		openingScore = readDouble(input);
	}
	...
	// Data for each skillKit skill is written on a single line.
...
// Read a 1.4 game record (the original format).
...
void GameRecord::read(std::istream & input)
{
    try
    {
		...
		if (recordFormat == "4.2.2")
		{
			read_v3_or_4(input, true);
		}
		else if (recordFormat == "3.0")
        {
			read_v3_or_4(input, false);
		}
...
// When this object is initialized, the opening and many other items are not yet known.
GameRecord::GameRecord()
	...
	, openingScore(-1.0)			// score will be [0..1]; -1 means we don't know it
...
void GameRecord::write(std::ostream & output)
{
    ...
    // TODO skip the snapshots for v3.0 and later
