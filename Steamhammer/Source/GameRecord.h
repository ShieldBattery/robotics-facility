...
protected:
    ...
    const std::string latestRecordFormat = "4.2.2";
	...
	double openingScore;					// evaluation at end of the opening, [0..1] or -1 if unavailable
	...
	void read_v3_or_4(std::istream & input, bool includeOpeningScore);
...
public:
	...
	int getFrameEnemyGetsAirUnits() const { return frameEnemyGetsAirUnits; };
	double getOpeningScore() const { return openingScore; };		// -1.0 (invalid) if the opening build is not over
