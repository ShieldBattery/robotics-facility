...
// A "combat" squad is any squad except certain special-purpose squads.
...
Squad::Squad(const std::string & name, size_t priority, bool isDrop)
	: _name(name)
	, _combatSquad(
		name != "Idle" &&
		name != "Irradiated" &&
...
void Squad::update()
{
	updateUnits();			// may delete bad units
	...
	// The Minelaying squad.
	if (_name == "Minelaying")
	{
		_microMinelaying.update();
		return;
	}
	...
	if (_order.getType() == SquadOrderTypes::Hold)
	{
		holdPosition();
		return;
	}
...
void Squad::setClusterStatus(UnitCluster & cluster)
{
    ...
    if (noCombatUnits(cluster))
    {
        ...
        else
        {
            ...
            moveCluster(cluster, finalRegroupPosition(cluster), false);
    ...
    else
    {
		...
		// The enemy may or may not be able to fight back.
...
void Squad::clusterCombat(const UnitCluster & cluster)
{
    ...
    else if (cluster.status == ClusterStatus::Regroup)
    {
        ...
        if (true /*_lurkerTactic == LurkerTactic::Aggressive*/)
        {
            //_microLurkers.setTactic(_lurkerTactic);
            _microLurkers.execute(cluster);
        }
        ...
        {
            _microLurkers.regroup(regroupPosition, cluster);
        }
    }
}
...
void Squad::addUnitsToMicroManagers()
{
    ...
    for (BWAPI::Unit unit : _units)
    {
        if (unit->isCompleted() && unit->exists() && unit->getHitPoints() > 0 && unit->getPosition().isValid())
        {
			...
			else if (_name == "Minelaying" && unit->getType() == BWAPI::UnitTypes::Terran_Vulture)
			{
				minelayingUnits.insert(unit);
			}
            ...
            // Melee units include firebats, which have range 32.
                ...
                unit->getType().groundWeapon().maxRange() > 0)              // and can attack: not a spellcaster
            {
                meleeUnits.insert(unit);
            }
	...
	_microMinelaying.setUnits(minelayingUnits);
...
bool Squad::needsToRegroup(UnitCluster & cluster)
{
    ...
    BWAPI::Unit defense = nearbyImmobileDefense(vanguard->getPosition(), cluster.air);
    if (defense)
    {
        // Don't retreat if we are in range of defense that is attacking, and we're close to it.
		// The distance limit is vital for sieged tanks and sensible for all immobile defense.
        if (defense->getOrder() == BWAPI::Orders::AttackUnit && defense->getDistance(vanguard) <= 5 * 32)
        {
			_regroupStatus = green + std::string("Go defense!");
			return false;
		}

		// If the defense is a bunker, it will never be firing. The units inside do that.
		// Handle the case. NOTE Steamhammer loads only marines into a bunker, no other types.
		if (defense->getType() == BWAPI::UnitTypes::Terran_Bunker)
		{
			BWAPI::Unitset contents = defense->getLoadedUnits();
			if (!contents.empty() && (*contents.begin())->getOrder() == BWAPI::Orders::AttackUnit)
			{
				_regroupStatus = green + std::string("Go bunker!");
				return false;
			}
		}
        ...
        if (defense->getDistance(cluster.center) < 5 * 32 &&
            cluster.center.getApproxDistance(enemyCluster->center) - defense->getDistance(enemyCluster->center) >= 2 * 32)
        {
    ...
    else
    {
		...
		if (vanguard->getDistance(finalRegroupPosition(cluster)) < 7 * 32)
        {
...
// Figure out where to retreat to. The cluster will group up there.
BWAPI::Position Squad::calcRegroupPosition(const UnitCluster & cluster) const
{
	const int riskRadius = 31 + (!cluster.air && the.info.enemyHasSiegeMode() ? 12 * 32 : 8 * 32);
	...
	//if (closestEnemy)
	//{
	//	BWAPI::Broodwar->drawCircleMap(closestEnemy->getPosition(), 5, BWAPI::Colors::Red);
	//	BWAPI::Broodwar->drawLineMap(cluster.center, closestEnemy->getPosition(), BWAPI::Colors::Red);
	//}
	...
	if (vanguard)
    {
		BWAPI::Unit nearest = nearbyImmobileDefense(vanguard->getPosition(), fleeingAir);
		if (nearest)
		{
			return seekSafePositionNear(cluster, nearest->getTilePosition());
		}
    }
	...
	{
		//return seekSafeAirRetreat(cluster, closestEnemy);
		//BWAPI::Broodwar->printf("muta retreat to safe position");
		return seekSafePositionNear(cluster, BWAPI::TilePosition(cluster.center));
	}

	// 4. Retreat to the location of the cluster unit not near the enemy which is
    ...
    if (regroup.isValid())
    {
		//BWAPI::Broodwar->printf("retreat to unit not near the enemy");
		return regroup;
    }

    // 4. Retreat to a base we own.
	return finalRegroupPosition(cluster);
}

// Return the rearmost position we should retreat to, which puts our "back to the wall".
// It is our main base or natural. 
BWAPI::Position Squad::finalRegroupPosition(const UnitCluster & cluster) const
{
	//BWAPI::Broodwar->printf("retreat home");

	// Retreat to the main base, unless we change our mind below.
    Base * base = the.bases.myMain();

	// If the cluster is in the main, and the main is under attack, don't retreat to the natural!
	if (base->inWorkerDanger() && base->isInBase(cluster.center))
	{
...
BWAPI::Unit Squad::nearbyImmobileGroundDefense(const BWAPI::Position & pos) const
{
    ...
    {
		// NOTE This doesn't check whether the bunker is loaded. There's no Filter for it.
		// TODO figure out a filter adaptor to make new Filters
        return BWAPI::Broodwar->getClosestUnit(
            pos,
            (BWAPI::Filter::GetType == BWAPI::UnitTypes::Terran_Bunker ||
    ...
    return BWAPI::Broodwar->getClosestUnit(
        pos,
        (BWAPI::Filter::GetType == BWAPI::UnitTypes::Zerg_Sunken_Colony ||
		 BWAPI::Filter::GetType == BWAPI::UnitTypes::Zerg_Lurker && BWAPI::Filter::IsBurrowed)
			&& BWAPI::Filter::IsOwned && BWAPI::Filter::IsCompleted,
        ImmobileDefenseRadius
    );
}
...
// In case we are fleeing enemy air, only include static air defense.
BWAPI::Unit Squad::nearbyImmobileDefense(const BWAPI::Position & pos, bool versusAir) const
{
    if (!versusAir)
    {
    ...
    BWAPI::Unit nearest = the.info.nearestAirStaticDefense(pos);		// at any distance
...
// Find the nearest enemy cluster that appears dangerous to the given cluster.
// It's not truly the calculation we want, but close enough.
// We expect one. But if none, return null.
const UnitCluster * Squad::findMenacingEnemyCluster(const UnitCluster & cluster) const
{
	std::function canHitUs =
		[cluster](const UnitCluster & c) -> bool { return cluster.air ? c.airDPF > 0.0 : c.groundDPF > 0.0; };

	return the.ops.getNearestEnemyClusterSuchThat(cluster.center, canHitUs);
}

// A cluster of our air units is fleeing the given enemy unit. Find a place for it to retreat to.
BWAPI::Position Squad::seekSafeAirRetreat(const UnitCluster & cluster, BWAPI::Unit enemyUnit) const
{
	UAB_ASSERT(enemyUnit, "no enemy unit");

	BWAPI::TilePosition center = BWAPI::TilePosition(cluster.center);

	// Air units stack. We need only a small safety margin from the danger zone.
	const int safeDist = 32 + 16 + 32 *
		(the.your.ever.count(BWAPI::UnitTypes::Terran_Goliath) + the.your.ever.count(BWAPI::UnitTypes::Protoss_Carrier) > 0 ? 8 : 7);

	int nHits = the.airHits(center);
	int dist = cluster.center.getApproxDistance(enemyUnit->getPosition());
	if (nHits == 0 && dist > safeDist)
	{
		return cluster.center;
	}
	BWAPI::TilePosition safestTile = center;
	int groundHeight = BWAPI::Broodwar->getGroundHeight(enemyUnit->getTilePosition());

	const int radius = 12;
	for (int x = std::max(0, center.x - radius); x <= std::min(BWAPI::Broodwar->mapWidth() - 1, center.x + radius); x += 4)
	{
		for (int y = std::max(0, center.y - radius); y <= std::min(BWAPI::Broodwar->mapHeight() - 1, center.y + radius); y += 4)
		{
			BWAPI::TilePosition t(x, y);
			int h = the.airHits(t);
			int d = enemyUnit->getDistance(TileCenter(t));

			// Adjust the distance to prefer safer locations, other things being equal.
			if (!enemyUnit->isFlying() && BWAPI::Broodwar->getGroundHeight(t) > groundHeight)
			{
				d += 32;		// on higher ground than the enemy, may be out of vision
			}
			if (!the.map.isWalkable(t))
			{
				d += 8;			// ground units can't go there (though the may be able to get close enough)
			}

			//BWAPI::Broodwar->drawTextMap(TileCenter(t), "%c%d %d", orange, h, d);
			if (h < nHits || h == nHits && d > dist)
			{
				if (h == 0 && d > safeDist)
				{
					//BWAPI::Broodwar->printf("  - safe air retreat SAFE PLACE");
					return TileCenter(t);
				}
				nHits = h;
				dist = d;
	...
	return TileCenter(safestTile);
}

// Look for a place near the given tile that the enemy can't hit.
// The given tile has immobile defense (static defense, sieged tank, burrowed lurker).
// The enemy unit may be null.
// The enemy unit may be null.
BWAPI::Position	Squad::seekSafePositionNear(const UnitCluster & cluster, const BWAPI::TilePosition & tile) const
{
	BWAPI::TilePosition safestTile = the.bases.myMain()->getTilePosition();

	if (cluster.air)
	{
		// For an air cluster, search over a fixed square area for the safest place.
		// Return a safe tile, or the safest tile in the square.
		// Prefer tiles closer to our main base.

		int nHits = clusterPositionCost(cluster, tile);
		if (nHits == 0)
		{
			return TileCenter(tile);
		}

		BWAPI::TilePosition clusterTile = BWAPI::TilePosition(cluster.center);
		//int dist = clusterTile.getApproxDistance(tile);
		int distHome = clusterTile.getApproxDistance(the.bases.myMain()->getTilePosition());

		const int radius = 8;
		for (int x = std::max(0, tile.x - radius); x <= std::min(BWAPI::Broodwar->mapWidth() - 1, tile.x + radius); x += 4)
		{
			for (int y = std::max(0, tile.y - radius); y <= std::min(BWAPI::Broodwar->mapHeight() - 1, tile.y + radius); y += 4)
			{
				BWAPI::TilePosition t(x, y);
				int h = clusterPositionCost(cluster, t);
				//int d = clusterTile.getApproxDistance(t);
				int dHome = clusterTile.getApproxDistance(the.bases.myMain()->getTilePosition());
				if (h < nHits || h == nHits && dHome < distHome) // || h == nHits && dHome == distHome && d <= dist)
				{
					if (h == 0)
					{
						return TileCenter(t);
	...
	{
		// For our ground units, we assume they can get to the given tile.
		// We search for a ground path from that tile to a safe place nearby.
		// Prefer directions toward our main base.
		...
		// Closer to our main is better.
		int dist = the.bases.myMain()->getDistance(TileCenter(tile));
		if (dist < 0)
		{
			//BWAPI::Broodwar->printf("seekSafePositionNear ground no search");
			return TileCenter(tile);		// oops, we are in a different partition, so give up
		}
		...
		{
			bool any = false;
			for (int x = std::max(0, safestTile.x - 1); x <= std::min(BWAPI::Broodwar->mapWidth() - 1, safestTile.x + 1); ++x)
			{
				for (int y = std::max(0, safestTile.y - 1); y <= std::min(BWAPI::Broodwar->mapHeight() - 1, safestTile.y + 1); ++y)
				{
					BWAPI::TilePosition t(x, y);
					if (t != safestTile && the.map.isWalkable(t))
					{
						int h = clusterPositionCost(cluster, t);
						int d = the.bases.myMain()->getDistance(TileCenter(t));
						if (d < 0)
						{
							continue;		// should never happen: tile is reachable from here but not from main
						}
						//BWAPI::Broodwar->drawTextMap(TileCenter(t), "%c%d %d", white, h, d);
						if (h < nHits || h == nHits && d < dist)
						{
							if (h == 0)
							{
								//BWAPI::Broodwar->printf("seekSafePositionNear ground found a safe tile");
								return TileCenter(t);
							}
							any = true;
							nHits = h;
				...
				if (!any)
				{
					//BWAPI::Broodwar->printf("seekSafePositionNear ground no more progress");
					break;
				}
			}
		}
		//BWAPI::Broodwar->printf("seekSafePositionNear best tile");
		//BWAPI::Broodwar->drawLineMap(cluster.center, TileCenter(safestTile), BWAPI::Colors::Yellow);
		return TileCenter(safestTile);
	}
}

// Estimate the cost to the cluster, in terms of hits suffered, if its center were at this position.
// The estimate is simple: Sample a few locations around the edge of the cluster and take the max.
// The passed-in tile position must be valid.
// It's used in searching for retreat paths.
int Squad::clusterPositionCost(const UnitCluster & cluster, const BWAPI::TilePosition & tile) const
{
	UAB_ASSERT(tile.isValid(), "bad tile");

	BWAPI::TilePosition center = BWAPI::TilePosition(cluster.center);
	int radius = 1 + cluster.radius / 32;		// add a margin of one tile

	int worst = the.hits(cluster.air, BWAPI::TilePosition(center.x, std::max(0, center.y - radius)));
	worst = std::max(worst, the.hits(cluster.air, BWAPI::TilePosition(center.x, std::min(BWAPI::Broodwar->mapHeight() - 1, center.y + radius))));
	worst = std::max(worst, the.hits(cluster.air, BWAPI::TilePosition(std::max(0, center.x - radius), center.y)));
	worst = std::max(worst, the.hits(cluster.air, BWAPI::TilePosition(std::min(BWAPI::Broodwar->mapWidth() - 1, center.x + radius), center.y)));
...
// Our unit is near some enemy unit, including non-combat units and buildings.
bool Squad::unitNearEnemy(BWAPI::Unit unit)
{
    ...
    // For each enemy unit, visible or not, including innocent buildings.
    for (const auto & kv : InformationManager::Instance().getUnitData(BWAPI::Broodwar->enemy()).getUnits())
    {
        const UnitInfo & ui(kv.second);

        if (!ui.goneFromLastPosition && ui.lastPosition.isValid())
        {
...
void Squad::setOrder(SquadOrder so)
{
	if (!so.isValid())
	{
		UAB_MESSAGE("bad squad order %s -> %d,%d type %c, radius %d", getName().c_str(), so.getPosition().x, so.getPosition().y, so.getCharCode(), so.getRadius());
		return;
	}
...
bool Squad::maybeWatch()
{
    ...
    for (BWAPI::Unit u : _units)
    {
        ...
        {
            if (u->canBurrow() &&
                u->getDistance(_order.getPosition()) <= positionFudge &&
        ...
        {
            // It's burrowed.
			if (u->getDistance(_order.getPosition()) > positionFudge)
			{
				the.micro.Unburrow(u);
				return true;
			}
...
// This is used only for air squads: Go to the order position and hold position.
void Squad::holdPosition()
{
	for (BWAPI::Unit airUnit : _units)
	{
		if (airUnit->getDistance(_order.getPosition()) <= _order.getRadius())
		{
			the.micro.HoldPosition(airUnit);
		}
		else
		{
			the.micro.MoveSafely(airUnit, _order.getPosition());
		}
	}
}
...
void Squad::loadTransport()
{
	for (BWAPI::Unit trooper : _units)
    {
        ...
        // which should be set to the transport's initial location.
		// The transport will move around as needed to pick up the troops.
        if (trooper->exists() && !trooper->isLoaded() && !trooper->isFlying())
        {
			if (trooper->getOrder() == BWAPI::Orders::Unburrowing)
			{
				// Nothing.
			}
			else if (trooper->isBurrowed())
			{
				trooper->unburrow();
			}
			else
    ...
    for (BWAPI::Unit transport : _microTransports.getUnits())
    {
        if (!transport->exists())
        {
            continue;
...
void Squad::stimIfNeeded()
{
    // Do we have stim?
    if (the.selfRace() != BWAPI::Races::Terran ||
        !the.self()->hasResearched(BWAPI::TechTypes::Stim_Packs))
    {
        return;
    }
...
int Squad::countUnits(BWAPI::UnitType type) const
{
	int count = 0;
	for (auto it = getUnits().begin(); it != getUnits().end(); ++it)
	{
		if ((*it)->getType() == type)
		{
			++count;
...
// The squad, or at least one unit of it, is very close to the order position--
// close enough to trigger a spider mine.
bool Squad::isAtOrderPosition() const
{
	return
		_vanguard &&
		_order.getPosition().isValid() &&
		_vanguard->getDistance(_order.getPosition()) < 64;
}

// Is every unit in the squad an overlord hunter (or a detector)?
// An overlord hunter is an air unit that is strong against overlords.
...
// Return a unit from the squad, any unit. Well, ground units preferred.
// This is used to check whether ground paths exist, using unit->hasPath().
BWAPI::Unit Squad::getSampleGroundUnit() const
{
...
// Return any scourge unit from the squad.
BWAPI::Unit Squad::getAnyScourge() const
{
	for (BWAPI::Unit u : _units)
	{
...
// Pixel distance from the given tile to the order position, using ground distance when possible.
...
// Pixel distance from the given position to the order position, using ground distance when possible.
