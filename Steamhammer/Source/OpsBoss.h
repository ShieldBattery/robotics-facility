...
namespace UAlbertaBot
{
    ...
    public:
		...
		size_t countUnits(BWAPI::UnitType type) const;
    ...
    class OpsBoss
    {
		bool _allHands;						// drop minor squads like Clearance so all units can fight together

		bool _attackAtMax;					// we should attack because we are maxed
		...
		bool enemyCannotWin() const;

    public:
        OpsBoss();
        void initialize();

		bool allHands() const { return _allHands; };
		bool attackAtMax();					// we should attack because we are maxed
		bool enemySeemsDead();
		bool wraithsAttackEnemyBuildings();
