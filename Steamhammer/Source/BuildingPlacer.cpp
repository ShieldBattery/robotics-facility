...
// Is the given position safe from immediate ground attack?
// We check the building's ground footprint plus one tile all around.
bool BuildingPlacer::isSafeHere(const BWAPI::TilePosition & pos, BWAPI::UnitType type) const
{
	for (int x = std::max(pos.x - 1, 0); x < std::min(pos.x + 1 + type.tileWidth(), BWAPI::Broodwar->mapWidth() - 1); ++x)
	{
		for (int y = std::max(pos.y - 1, 0); y < std::min(pos.y + 1 + type.tileHeight(), BWAPI::Broodwar->mapHeight() - 1); ++y)
		{
			if (the.groundHits(BWAPI::TilePosition(x, y)) > 0)
			{
				return false;
			}
		}
...
bool BuildingPlacer::canBuildHere(const BWAPI::TilePosition & position, const Building & b) const
{
    return
        ...
        // The enemy army is not in range to hit the building location.
		isSafeHere(position, b.type);
}
...
// The building type can have an addon.
// Look for an existing addon that we can build it next to.
BWAPI::TilePosition BuildingPlacer::findExistingAddonLocation(const Building & b) const
{
	BWAPI::UnitType addonType;
	if (b.type == BWAPI::UnitTypes::Terran_Factory)
	{
		addonType = BWAPI::UnitTypes::Terran_Machine_Shop;
	}
	else if (b.type == BWAPI::UnitTypes::Terran_Starport)
	{
		addonType = BWAPI::UnitTypes::Terran_Control_Tower;
	}
	else if (b.type == BWAPI::UnitTypes::Terran_Science_Facility)
	{
		// Since ghosts aren't supported (yet).
		addonType = BWAPI::UnitTypes::Terran_Physics_Lab;
	}
	else
	{
		return BWAPI::TilePositions::None;
	...
	BWAPI::Unitset addons = BWAPI::Broodwar->getUnitsInRadius(
		BWAPI::Position(b.desiredPosition),
		12 * 32,
		BWAPI::Filter::GetType == addonType
	);

	for (BWAPI::Unit addon : addons)
	{
		// This works for all buildings that can take addons.
		// They are all 4x3 tiles (and all addons are 2x2 tiles).
		BWAPI::TilePosition tile = addon->getTilePosition() + BWAPI::TilePosition(-4, -1);

		// Does not check for free space around the building location.
		// Normally there is space, but in principle this could cause blockage.
...
BWAPI::TilePosition BuildingPlacer::findSpecialLocation(const Building & b) const
{
	...
	else if (b.type.canBuildAddon() && b.type != BWAPI::UnitTypes::Terran_Command_Center)
	{
		tile = findExistingAddonLocation(b);
		if (!tile.isValid())
		{
			tile = findGroupedLocation(b);
		}
...
void BuildingPlacer::reserveTiles(const Building & b)
{
	setReserve(b.finalPosition, b.type.tileWidth(), b.type.tileHeight(), true);
}
...
// Are all tiles the building will cover explored?
// Used in landing floating buildings that may want to land far away in the dark.
// (In constructing buildings, the worker explores the area when it arrives.)
bool BuildingPlacer::isExplored(const Building & b) const
{
	for (int x = b.finalPosition.x; x < b.finalPosition.x + b.type.tileWidth(); ++x)
	{
		for (int y = b.finalPosition.y; y < b.finalPosition.y + b.type.tileHeight(); ++y)
		{
...
// The building object's macroLocation must be set.
// It's mostly like other buildings, except that the "Anywhere" default is different.
BWAPI::TilePosition BuildingPlacer::getExpoLocationTile(const Building & b) const
{
    if (b.macroLocation == MacroLocation::Anywhere)
    {
        if (the.bases.myNatural() &&
			...
			buildingOK(b, the.bases.myNatural()->getTilePosition()))
        {
            return the.bases.myNatural()->getTilePosition();
        }
        else
        {
			return getMacroLocationTile(MacroLocation::Expo);
        }
    }

    return getMacroLocationTile(b.macroLocation);
}
