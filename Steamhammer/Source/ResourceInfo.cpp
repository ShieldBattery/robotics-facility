...
void ResourceInfo::updateAmount(int a)
{
    amount = a;
...
// NOTE Terran scan causes mineral patches to be isVisible() but not accessible,
//      so that they are falsely marked "destroyed". This code undestroys
//      them so they continue to show the correct amounts.
void ResourceInfo::updateMinerals()
{
	if (initialUnit->isVisible())
	{
		updateAmount(initialUnit->getResources());
		destroyed = false;
	}
