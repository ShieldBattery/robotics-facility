...
SquadOrder::SquadOrder(SquadOrderTypes type, const std::string & status)
	: _type(type)
	, _position(BWAPI::Positions::Origin)
	, _base(nullptr)
	, _radius(0)
...
const char SquadOrder::getCharCode() const
{
	...
	case SquadOrderTypes::LayMines:			return 'M';
...
// Does the order look legal?
bool SquadOrder::isValid() const
{
	return
		_type == SquadOrderTypes::None ||
		_position.isValid() && _type >= SquadOrderTypes::None && _type <= SquadOrderTypes::LAST && _radius >= 0;
}
