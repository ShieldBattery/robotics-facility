...
void GameCommander::update()
{
	...
	the.micro.update();
	the.microStaticDefense.update();
	the.buildingPilot.update();
    _timerManager.stopTimer(TimerManager::Micro);

    // Last so that it can delay tasks that may take too long.
	// NOTE Delaying is not yet implemented--or needed.
...
{
	if (!the.production.isOutOfBook())
	{
		// The game ended while we are still in book. Set the opening score to 0 or 1.
		// The other case is handled in ProductionManager::goOutOfBookAndClearQueue().
		the.oppModel().setOpeningScore(isWinner ? 1.0 : 0.0);
	}
    the.oppModel().setWin(isWinner);
	the.oppModel().write();

	the.evaluator.learn(isWinner);
	the.evaluator.write();
...
void GameCommander::drawGameInformation(int x, int y)
{
	...
	std::string screenText = "%c%s %c%d-%d %c%s";

	// Evaluation display.
	double eval = 0.0;
	if (the.evaluator.hasEvaluation())
	{
		screenText += " %ceval %c%1.2f";
		eval = the.evaluator.getLastEvaluation();
	}

	const OpponentModel::OpponentSummary & summary = OpponentModel::Instance().getSummary();
    BWAPI::Broodwar->drawTextScreen(x, y, screenText.c_str(),
		...
		yellow, white, eval);		// if unneeded, the extra parameters do not hurt
    y += 12;
    ...
    BWAPI::Broodwar->drawTextScreen(x, y, "%c%s%s%s%s %c%d-%d %c%s",
		yellow,
		...
		orange, the.opsStrategy.getAggressionString()			// Defend/Contain/Attack
		);
...
void GameCommander::drawKills()
{
	if (!Config::Debug::DrawKills)
	{
		return;
	}
	...
	for (BWAPI::Unit u : BWAPI::Broodwar->getAllUnits())
	{
		if (u->getKillCount() > 0)
		{
			std::string plural = u->getKillCount() > 1 ? "s" : "";
			BWAPI::Broodwar->drawTextMap(
				u->getPosition() + BWAPI::Position(-10, Config::Debug::DrawUnitOrders ? 18 : 10),
				"%c%d kill%s", yellow, u->getKillCount(), plural.c_str()
			);
		}
	}
}

// Only draws enemy unit velocities.
void GameCommander::drawVelocities()
{
	if (!Config::Debug::DrawVelocities)
	{
		return;
	}
	...
	for (const auto & kv : the.info.getUnitData(the.enemy()).getUnits())
	{
		...
		const double frames = 12.0;					// draw toward position this many frames in the future
		v2 toward = v2(ui.lastPosition) + ui.lastVelocity * frames;
		BWAPI::Broodwar->drawLineMap(ui.lastPosition, BWAPI::Position(toward), BWAPI::Colors::Grey);
	}
}
...
void GameCommander::drawUnitOrders()
{
    ...
    for (BWAPI::Unit unit : BWAPI::Broodwar->getAllUnits())
    {
		...
		// NOTE If DrawKills is turned on, it overlaps any secondary order on the screen.
		//if (unit->getSecondaryOrder() != BWAPI::Orders::Nothing)
		//{
		//	BWAPI::Broodwar->drawTextMap(x, y + 20, "%cand %c%s", white, cyan, unit->getSecondaryOrder().getName().c_str());
		//}
    }
}
