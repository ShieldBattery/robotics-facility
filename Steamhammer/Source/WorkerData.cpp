...
void WorkerData::setWorkerJobMinerals(BWAPI::Unit unit, Base * base)
{
	BWAPI::Unit depot = base->getDepot();

	clearPreviousJob(unit);
	workerJobMap[unit] = Minerals;

	depotWorkerCount[depot] += 1;

	workerDepotMap[unit] = depot;

	BWAPI::Unit mineralToMine = getMineralToMine(unit, base);
	if (!mineralToMine)
	{
...
int WorkerData::getNumWorkersWithJob(WorkerJob job) const
{
	int num = 0;
	for (BWAPI::Unit unit : workers)
	{
		if (workerJobMap.at(unit) == job)
		{
			++num;
		}
	}
...
{
	return getNumWorkersWithJob(WorkerData::Minerals);
}

int WorkerData::getNumGasWorkers() const
{
	return getNumWorkersWithJob(WorkerData::Gas);
}

int WorkerData::getNumReturnCargoWorkers() const
{
	return getNumWorkersWithJob(WorkerData::ReturnCargo);
}

int WorkerData::getNumCombatWorkers() const
{
	return getNumWorkersWithJob(WorkerData::Combat);
}

int WorkerData::getNumRepairWorkers() const
{
	return getNumWorkersWithJob(WorkerData::Repair);
}

int WorkerData::getNumIdleWorkers() const
{
	return getNumWorkersWithJob(WorkerData::Idle);
}

int WorkerData::getNumPostedWorkers() const
{
    return int(workerPostMap.size());
}

bool WorkerData::anyUnblocker() const
{
	return getNumWorkersWithJob(WorkerData::Unblock);
}
...
// This takes into account mineral patches that have been mined out
// and those that are under fire by the enemy and cannot be mined.
bool WorkerData::baseIsFull(Base * base) const
{
	UAB_ASSERT(base, "no base");

	int assignedWorkers = getNumAssignedWorkers(base->getDepot());
	int mineralsNearDepot = base->getRemainingSafeMinerals().size();

	// Mineral locking ensures that 2 workers per mineral patch are always enough (on normal maps).
	// The configuration file may specify a lower limit.
	return
		assignedWorkers >= 2 * mineralsNearDepot ||
		assignedWorkers >= int(Config::Macro::WorkersPerPatch * mineralsNearDepot + 0.5);
}

// NOTE Used only in the debug display.
BWAPI::Unitset WorkerData::getMineralPatchesNearDepot(BWAPI::Unit depot)
{
    ...
    {
        if (unit->getType() == BWAPI::UnitTypes::Resource_Mineral_Field &&
			the.info.getResourceAmount(unit) > 0 &&
			unit->getDistance(depot) < radius)
        {
...
// Count mineral patches near the resource depot.
...
BWAPI::Unit WorkerData::getMineralToMine(BWAPI::Unit worker, Base * base)
{
    ...
    if (depot)
    {
        BWAPI::Unitset mineralPatches = base->getRemainingSafeMinerals();
