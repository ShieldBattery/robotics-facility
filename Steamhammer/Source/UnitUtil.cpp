...
// Zerg unit (not building) morphed from another, not spawned from a larva.
...
bool UnitUtil::IsMorphedType(BWAPI::UnitType type)
{
	return IsMorphedBuildingType(type) || IsMorphedUnitType(type);
}
...
bool UnitUtil::IsNearlyCompletedResourceDepot(BWAPI::Unit unit, int framesLeft)
{
    return
        ...
        unit->getType().isResourceDepot() &&
        (unit->isCompleted() ||
         unit->getRemainingBuildTime() <= framesLeft - 12 ||		// -12 since workers occasionally arrive early
...
bool UnitUtil::IsCombatSimUnit(BWAPI::Unit unit)
{
    if (!unit->isCompleted() ||
        ...
        unit->isLockedDown() ||
        unit->isMaelstrommed() ||
        unit->isUnderDisruptionWeb() ||		// though it could move out from under
        unit->isStasised())
    {
        return false;
    }
...
// NOTE Spider mines are combat sim units but are not squad combat units.
...
{
    // No workers, buildings, spider mines, or carrier interceptors (which are not controllable).
...
// Check whether a unit is a unit the enemy controls.
bool UnitUtil::IsValidEnemyUnit(BWAPI::Unit unit)
{
	return
		unit &&
		unit->exists() &&
		...
		unit->getType() != BWAPI::UnitTypes::Unknown &&
		!unit->getType().isSpell() &&
		unit->getPlayer() == BWAPI::Broodwar->enemy();
}

bool UnitUtil::BuildingIsBusy(BWAPI::Unit unit)
{
    return
		unit->isTraining() ||
		unit->isUpgrading() ||
...
// The unit-completed test tells our units that uncompleted enemy static defense can't shoot.
// Call this for enemy units (our own units are always completed before we think about them).
bool UnitUtil::CanAttackAir(BWAPI::Unit attacker)
{
    return attacker->isCompleted() && TypeCanAttackAir(attacker->getType());
}

// Call this (not CanAttackAir) for our units and any time that only the type is available.
...
{
    return attacker->isCompleted() && TypeCanAttackGround(attacker->getType());
}

// Call this (not CanAttackGround) for our units and any time that only the type is available.
...
// Damage per frame.
// NOTE This does not account for unit sizes or damage type.
//      It's a generic "how hard does it hit?" value.
double UnitUtil::DPF(BWAPI::Unit attacker, BWAPI::Unit target)
{
    BWAPI::WeaponType weapon = GetWeapon(attacker, target);

    // NOTE Zerglings are the only unit with a cooldown upgrade.
	// NOTE Marine stim decreases cooldown. We don't take it into account.
...
// We check unit->isFlying() because unitType->isFlyer() is not useful
...
int UnitUtil::GetAttackRange(BWAPI::Unit attacker, BWAPI::Unit target)
{
    ...
    {
        if (attacker->getPlayer()->getUpgradeLevel(BWAPI::UpgradeTypes::U_238_Shells))
        {
            return 6 * 32;
        }
        return 5 * 32;
...
// My weapon range in pixels.
// Returns 0 if the attacker does not have a way to attack the target.
// NOTE Does not check whether our reaver, carrier, or bunker has units inside that can attack.
int UnitUtil::GetMyAttackRange(BWAPI::UnitType attacker, BWAPI::UnitType target)
{
	// Reavers, carriers, and bunkers have "no weapon" but still have an attack range.
	...
	if (attacker == BWAPI::UnitTypes::Terran_Bunker)
	{
		if (the.self()->getUpgradeLevel(BWAPI::UpgradeTypes::U_238_Shells))
		{
			return 6 * 32;
		}
		return 5 * 32;
	...
	return the.self()->weaponMaxRange(weapon);
}

// Enemy weapon range in pixels.
// Returns 0 if the attacker does not have a way to attack the target.
// NOTE Does not check whether our reaver, carrier, or bunker has units inside that can attack.
int UnitUtil::GetYourAttackRange(BWAPI::UnitType attacker, BWAPI::UnitType target)
{
	// Reavers, carriers, and bunkers have "no weapon" but still have an attack range.
	if (attacker == BWAPI::UnitTypes::Protoss_Reaver && !target.isFlyer())
	{
	...
	if (attacker == BWAPI::UnitTypes::Terran_Bunker)
	{
		if (the.enemy()->getUpgradeLevel(BWAPI::UpgradeTypes::U_238_Shells))
		{
			return 6 * 32;
		}
		return 5 * 32;
	...
	return the.enemy()->weaponMaxRange(weapon);
}

// Get the maximum attack range in pixels of the attacker, assuming all range upgrades.
	// Range is zero if the attacker cannot attack the target at all.
int UnitUtil::GetMaxAttackRange(BWAPI::UnitType attacker, BWAPI::UnitType target)
{
	// Reavers, carriers, and bunkers have "no weapon" but still have an attack range.
	if (attacker == BWAPI::UnitTypes::Terran_Bunker)
	{
		return 6 * 32;		// assume marine attack range is upgraded
	}
	if (attacker == BWAPI::UnitTypes::Protoss_Reaver && !target.isFlyer())
	{
		return 8 * 32;
...
{
    return std::max(
        GetMaxAttackRange(type, BWAPI::UnitTypes::Terran_Marine),   // range vs. ground
        GetMaxAttackRange(type, BWAPI::UnitTypes::Terran_Wraith)    // range vs. air
    );
}
...
int UnitUtil::FramesToReachAttackRange(BWAPI::Unit attacker, BWAPI::Unit target)
{
    ...
    int distanceToFiringRange =
		std::max(attacker->getDistance(target) - UnitUtil::GetAttackRange(attacker, target), 0);
...
int UnitUtil::GetWeaponDamageToWorker(BWAPI::Unit attacker)
{
	...
	int damage = attacker->getPlayer()->damage(weapon);
...
// NOTE The list may be incomplete. And it deliberately ignores spell casting.
...
// It works for terran scanner sweep, though it slightly overestimates its radius.
// NOTE The position MUST be valid.
bool UnitUtil::EnemyDetectorInRange(BWAPI::Position pos)
{
    if (the.airHitsFixed.at(pos))
    {
    ...
    return nullptr != BWAPI::Broodwar->getClosestUnit(
        pos,
        BWAPI::Filter::IsDetector && BWAPI::Filter::IsEnemy && BWAPI::Filter::IsFlyer && !BWAPI::Filter::IsBlind && BWAPI::Filter::IsPowered,
        11 * 32
    );
}
...
// Can we detect a unit at this position?
// This is nearly accurate, possibly off at the edges.
// It works for terran scanner sweep, though it slightly overestimates its radius.
bool UnitUtil::FriendlyDetectorInRange(BWAPI::Position pos)
{
	if (!pos.isValid())
	{
		return false;
	...
	// Turrets, cannons, spores.
	if (BWAPI::Broodwar->getClosestUnit(
		pos,
		BWAPI::Filter::IsDetector && BWAPI::Filter::IsOwned && !BWAPI::Filter::IsFlyer && BWAPI::Filter::IsPowered,
		7 * 32
	))
	{
		return true;
	...
	// Vessels, scans (approximately), cannons, overlords.
	return nullptr != BWAPI::Broodwar->getClosestUnit(
		pos,
		BWAPI::Filter::IsDetector && BWAPI::Filter::IsOwned && BWAPI::Filter::IsFlyer && !BWAPI::Filter::IsBlind,
		11 * 32
	);
}
