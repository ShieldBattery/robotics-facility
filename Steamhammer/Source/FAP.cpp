...
// This version is updated to understand dark swarm and ensnare, in an approximate way.
// There are a few bug fixes and other improvements.

// NOTE FAP does not look at UnitInfo.goneFromLastPosition. The flag is always false
...
namespace UAlbertaBot {
    ...
    void FastAPproximation::unitsim(const FastAPproximation::FAPUnit &fu, std::vector <FastAPproximation::FAPUnit> &enemyUnits) {
        ...
        if (closestEnemy != enemyUnits.end() &&
            ...
            if (closestEnemy->flying) {
                ...
                if (fu.elevation >= 0 && closestEnemy->elevation > fu.elevation && fu.groundMaxRange > 32 * 32)
                    fu.attackCooldownRemaining += fu.groundCooldown;
            }
    ...
    void FastAPproximation::medicsim(const FAPUnit & fu, std::vector<FAPUnit> &friendlyUnits) {
        ...
        int closestDist = InfiniteDistanceSquared;
	...
	// The suicide units are spider mines, scarabs, scourge, and infested terrans.
	// Any scarabs are existing ones passed in from outside, not generated by fap.
	// Returns whether the suicide unit suicided; if so, the caller deletes it from the unit set.
		...
		player(ui.player)
	{
		...
		if (ui.type == BWAPI::UnitTypes::Protoss_Carrier) {
			...
			// Assume the bunker is loaded with 4 marines.
		...
		// Only flying buildings can land, and they are excluded from the sim, so just check the type.
		// elevation is initialized to -1 in the constructor.
        if (!ui.type.isFlyer()) {
    ...
    const FastAPproximation::FAPUnit &FastAPproximation::FAPUnit::operator=(const FAPUnit & other) const {
        x = other.x, y = other.y;
        health = other.health, maxHealth = other.maxHealth, armor = other.armor;
        shields = other.shields, shieldArmor = other.shieldArmor, maxShields = other.maxShields;
        speed = other.speed, flying = other.flying, elevation = other.elevation, underSwarm = other.underSwarm, unitSize = other.unitSize;
		...
		unitType = other.unitType; player = other.player; isOrganic = other.isOrganic;
