...
class WorkerManager
{
    ...
    static const int ThisBaseRange = 14 * 32;
    ...
    Base * getClosestNonFullBase(BWAPI::Unit worker);		// only if it can accept more mineral workers
...
public:
    ...
    bool		isCollectingGas() const			{ return _collectGas; };
	...
	bool		isBadlyHurt(BWAPI::Unit worker) const;

    BWAPI::Unit getBuilder(const Building & b);
    void        setBuildWorker(BWAPI::Unit worker);
	BWAPI::Unit	getLongDistanceWorker(const BWAPI::Position & pos);
