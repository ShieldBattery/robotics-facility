...
void ParseUtils::ParseConfigFile(const std::string & filename)
{
    ...
    if (doc.HasMember("Debug") && doc["Debug"].IsObject())
    {
		...
		JSONTools::ReadBool("DrawKills", debug, Config::Debug::DrawKills);
		JSONTools::ReadBool("DrawVelocities", debug, Config::Debug::DrawVelocities);
		...
		JSONTools::ReadBool("DrawEvaluation", debug, Config::Debug::DrawEvaluation);
		JSONTools::ReadBool("DrawForecast", debug, Config::Debug::DrawForecast);
	}
    ...
    if (doc.HasMember("IO") && doc["IO"].IsObject())
    {
		...
		Config::IO::UseEvaluator = GetBoolByRace("UseEvaluator", io);
		Config::IO::UpdateEvaluator = GetBoolByRace("UpdateEvaluator", io);
	}
    ...
    if (doc.HasMember("Strategy") && doc["Strategy"].IsObject())
    {
        ...
        if (strategy.HasMember("Strategies") && strategy["Strategies"].IsObject())
        {
            ...
            for (rapidjson::Value::ConstMemberIterator itr = strategies.MemberBegin(); itr != strategies.MemberEnd(); ++itr)
            {
				...
				// Only parse the ones that are for our current race.
				if (strategyRace == the.selfRace())
				{
					...
					bool isRush = false;
					if (val.HasMember("Moveout") && val["Moveout"].IsString())
					{
						isRush = std::string("Rush") == val["Moveout"].GetString();
					}

					BuildOrder buildOrder(strategyRace, isRush);
...
void ParseUtils::ParseTextCommand(const std::string & commandString)
{
    ...
    if (command == "/set")
    {
		...
		else if (variableName == "drawkills") { Config::Debug::DrawKills = GetBoolFromString(val); }
		else if (variableName == "drawvelocities") { Config::Debug::DrawVelocities = GetBoolFromString(val); }
		...
		else if (variableName == "drawevaluation") { Config::Debug::DrawEvaluation = GetBoolFromString(val); }
		else if (variableName == "drawforecast") { Config::Debug::DrawForecast = GetBoolFromString(val); }
    ...
    const rapidjson::Value * strategyCombos)
{
    if (item.IsString())
    {
        ...
        return _LookUpStrategyCombo(stratName, mapWeightString, raceString, strategyCombos);
    }
