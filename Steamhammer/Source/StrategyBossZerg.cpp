...
void StrategyBossZerg::updateGameState()
{
	...
	// NOTE The exact check depends on the order of upgrades!
    hasUltraUps =
...
bool StrategyBossZerg::enemyIsAllAir() const
{
    ...
    {
        // NOTE A floating command center does not count as a base.
        return false;
    }
...
int StrategyBossZerg::devourerLimit() const
{
    int enemyAir =
        ...
        3 * the.your.seen.count(BWAPI::UnitTypes::Terran_Battlecruiser) +
...
bool StrategyBossZerg::nextInQueueIsUseless(BuildOrderQueue & queue) const
{
    ...
    if (act.isUpgrade())
    {
		...
		if (upInQueue == BWAPI::UpgradeTypes::Chitinous_Plating || upInQueue == BWAPI::UpgradeTypes::Anabolic_Synthesis)
		{
			return !hasUltra ||
    ...
    if (nextInQueue.isBuilding())
    {
        ...
        {
            return nLairs == 0 ||
				hasHiveTech ||			// don't need another (may change this rule if our existing hive is under attack)
        ...
        {
            return
				!hasPool && the.my.all.count(BWAPI::UnitTypes::Zerg_Spawning_Pool) == 0 && !isBeingBuilt(BWAPI::UnitTypes::Zerg_Spawning_Pool) ||
				nDrones <= 1 || nHatches == 0;
        }
        ...
        {
			return
				// Lost the prerequisite.
				!hasPool && the.my.all.count(BWAPI::UnitTypes::Zerg_Spawning_Pool) == 0 ||
				// Don't make hydra den in ZvZ unless the opening calls for it AND uses it to make hydras.
				// NOTE Consider removing this if we ever implement a ZvZ hydra strategy!
				!outOfBook && _enemyRace == BWAPI::Races::Zerg && !queue.anyInQueue(BWAPI::UnitTypes::Zerg_Hydralisk);
        }
    ...
    {
        // We lost the tech. It's OK to go over the queen count limit.
        return
            the.my.all.count(BWAPI::UnitTypes::Zerg_Queens_Nest) == 0;
    }
...
// - or the queue ends
bool StrategyBossZerg::queueSupplyIsOK(BuildOrderQueue & queue) const
{
        ...
        OpponentModel::Instance().getEnemyPlan() == OpeningPlan::Proxy)
    {
        if (!_droneToHide && WorkerManager::Instance().getNumPostedWorkers() == 0)
        {
            ...
            if (ScoutManager::Instance().getWorkerScout())
            {
                ...
                ScoutManager::Instance().sendWorkerScoutHome();
...
void StrategyBossZerg::makeUrgentReaction(BuildOrderQueue & queue)
{
    ...
    if (hasSpire && nGas > 0 &&
        ...
        the.your.seen.count(BWAPI::UnitTypes::Terran_Battlecruiser) <= 4 &&
    ...
    else if (outOfBook && queue.getNextGasCost(4) > gas && !WorkerManager::Instance().isCollectingGas())
    {
        ...
        else
        {
            ...
            ScoutManager::Instance().sendWorkerScoutHome();   // don't throw off the drone count
...
void StrategyBossZerg::analyzeExtraDrones()
{
    ...
    int enemyBases = the.bases.baseCount(the.enemy());
    if (enemyBases > 1)
    {
        extraDrones += (enemyBases - 1) * 300.0 / droneCost;
    }
...
void StrategyBossZerg::setAvailableTechUnits(std::array<bool, int(TechUnit::Size)> & available) const
{
    ...
    // Hive tech.
    available[int(TechUnit::Ultralisks)] = hasUltra && hasUltraUps && nGas >= 3;
...
void StrategyBossZerg::recommendTech()
{
    if (_enemyRace == BWAPI::Races::Terran)
    {
		...
		const int nAir = enemyAirToAir + 3 * the.your.seen.count(BWAPI::UnitTypes::Terran_Battlecruiser);
        ...
        // Usually make a queen if there is an enemy base, because we may be able to infest a command center.
		// NOTE If there are a lot of wraiths, we'll recommend ensnare and make a queen anyway.
		const int airDanger = enemyAirToAir + (the.info.enemyHasIrradiate() ? the.your.seen.count(BWAPI::UnitTypes::Terran_Science_Vessel) : 0);
		_recommendParasite = the.bases.baseCount(the.enemy()) > 0 && airDanger <= 8 ? 1 : 0;
        _recommendEnsnare =
    ...
    else if (_enemyRace == BWAPI::Races::Protoss)
    {
		...
		_recommendDefensiveSpire =
			...
			the.info.enemyHas8Interceptors() ||
			the.your.seen.count(BWAPI::UnitTypes::Protoss_Reaver) > 0 ||
			enemyAirToAir > 0 && enemyAirToAir <= 8;
...
void StrategyBossZerg::vTerranTechScores(const PlayerSnapshot & snap)
{
	...
	// Special case: If we're facing battlecruisers, force a minimum hydra count.
	const int nBattleCruisers = snap.count(BWAPI::UnitTypes::Terran_Battlecruiser);
	const int hydrasPerCruiser = 5;
	if (nBattleCruisers >= 3 && nHydras < hydrasPerCruiser * nBattleCruisers)
	{
		techScores[int(TechUnit::Hydralisks)] = 100;
		defilerScore += 10;
		return;
	}

    // Bias.
    techScores[int(TechUnit::Mutalisks)]  =   4;   // default lair tech
	...
	// Irradiate adjustment for some unit types.
	int irradiate = the.info.enemyHasIrradiate();		// 0 or 1
    ...
    for (const std::pair<BWAPI::UnitType, int> & unitCount : snap.getCounts())
    {
            ...
            type == BWAPI::UnitTypes::Terran_Ghost)
        {
            ...
            techScores[int(TechUnit::Lurkers)] += count * (2 - irradiate);
            techScores[int(TechUnit::Guardians)] += count;
            techScores[int(TechUnit::Ultralisks)] += count * 3;
			defilerScore += 1;		// added once per type!
        }
        else if (type == BWAPI::UnitTypes::Terran_Firebat)
        {
            techScores[int(TechUnit::Zerglings)] -= count * 2;
            techScores[int(TechUnit::Mutalisks)] += count * (3 - irradiate);
            techScores[int(TechUnit::Lurkers)] += count * (2 - irradiate);
            techScores[int(TechUnit::Guardians)] += count * (2 - irradiate);
            ...
            type == BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode)
        {
			...
			int lurkerTankWeight = 6;
			if (the.your.seen.count(BWAPI::UnitTypes::Terran_Science_Vessel) > 0)
			{
				lurkerTankWeight = 10;
			}
			else if (the.info.enemyHasMobileDetection())
			{
				lurkerTankWeight = 8;		// only scans are known
			}
			techScores[int(TechUnit::Lurkers)] -= count * lurkerTankWeight;
        }
        ...
        {
            techScores[int(TechUnit::Hydralisks)] += count * 10;
            techScores[int(TechUnit::Mutalisks)] -= count * 2;
            techScores[int(TechUnit::Guardians)] -= count * 10;
            techScores[int(TechUnit::Devourers)] += count * 8;
            techScores[int(TechUnit::Ultralisks)] -= count * 8;
            defilerScore += 8;
        }
	...
	if (the.your.seen.count(BWAPI::UnitTypes::Terran_Marine) >= 24)
	{
		defilerScore += 6;
	}
	if (the.your.seen.count(BWAPI::UnitTypes::Terran_Physics_Lab) > 0)
	{
		defilerScore += 4;		// battlecruisers are likely
	}

	// Adjustments for units that Steamhammer has never seen from this opponent.
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode) &&
		the.forecast.neverSeen(BWAPI::UnitTypes::Terran_Siege_Tank_Siege_Mode))
	{
		techScores[int(TechUnit::Lurkers)] += 12;
		techScores[int(TechUnit::Ultralisks)] += 20;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Terran_Science_Vessel))
	{
		techScores[int(TechUnit::Lurkers)] += 10;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Terran_Wraith) &&
		the.forecast.neverSeen(BWAPI::UnitTypes::Terran_Valkyrie))
	{
		techScores[int(TechUnit::Mutalisks)] += 10;
		techScores[int(TechUnit::Guardians)] += 15;
	}
}

void StrategyBossZerg::vProtossTechScores(const PlayerSnapshot & snap)
{
    _wantAirArmor = snap.count(BWAPI::UnitTypes::Protoss_Corsair) >= 3;
    ...
    for (const std::pair<BWAPI::UnitType, int> & unitCount : snap.getCounts())
    {
        ...
        if (!type.isWorker() && !type.isBuilding() && type != BWAPI::UnitTypes::Protoss_Interceptor)
        {
            ...
            else if (type.isFlyer())
            {
				...
				{
					// Corsairs do stacking area damage.
					// TODO should count enemy's attack upgrades
					int danger = std::max(0, count - the.self()->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Flyer_Carapace));
					int baseScore = danger * danger;
					techScores[int(TechUnit::Mutalisks)] -= baseScore;
					techScores[int(TechUnit::Guardians)] -= 2 * baseScore;
					techScores[int(TechUnit::Devourers)] += baseScore;
				}
	...
	// Adjustments for units that Steamhammer has never seen from this opponent.
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Protoss_Reaver))
	{
		techScores[int(TechUnit::Lurkers)] += 10;
		techScores[int(TechUnit::Ultralisks)] += 15;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Protoss_Observer))
	{
		techScores[int(TechUnit::Lurkers)] += 15;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Protoss_Archon))
	{
		techScores[int(TechUnit::Mutalisks)] += 5;
		techScores[int(TechUnit::Ultralisks)] += 10;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Protoss_Corsair))
	{
		techScores[int(TechUnit::Mutalisks)] += 10;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Protoss_Scout))
	{
		techScores[int(TechUnit::Mutalisks)] += 4;
		techScores[int(TechUnit::Guardians)] += 8;
	}
}
...
if (type == BWAPI::UnitTypes::Zerg_Zergling)
	...
	// Adjustments for units that Steamhammer has never seen from this opponent.
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Zerg_Queen))
	{
		techScores[int(TechUnit::Lurkers)] += 15;
		techScores[int(TechUnit::Ultralisks)] += 15;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Zerg_Mutalisk))
	{
		techScores[int(TechUnit::Lurkers)] += 15;
	}
	if (the.forecast.neverSeen(BWAPI::UnitTypes::Zerg_Scourge))
	{
		techScores[int(TechUnit::Mutalisks)] += 10;
	}
}
...
// NOTE lookaheadFrames was planned but is unused so far.
void StrategyBossZerg::calculateTechScores(int lookaheadFrames)
{
	...
	// Attack and defense upgrades promote sticking with the same unit types.
    int meleeUpScore =
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Melee_Attacks) +
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Carapace);
    techScores[int(TechUnit::Zerglings)] += 4 * meleeUpScore;
    techScores[int(TechUnit::Ultralisks)] += 6 * meleeUpScore;

    int missileUpScore =
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Missile_Attacks) +
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Carapace);
    techScores[int(TechUnit::Hydralisks)] += 4 * missileUpScore;
    techScores[int(TechUnit::Lurkers)] += 6 * missileUpScore;

    int airUpScore =
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Flyer_Attacks) +
        _self->getUpgradeLevel(BWAPI::UpgradeTypes::Zerg_Flyer_Carapace);
    techScores[int(TechUnit::Mutalisks)] += 5 * airUpScore;
    techScores[int(TechUnit::Guardians)] += 7 * airUpScore;
    techScores[int(TechUnit::Devourers)] += 3 * airUpScore;

	// If we have mutas, but only a few, add to the muta score.
	if (nMutas > 0 && hasSpire && nMutas < 7)
	{
		if (_enemyRace == BWAPI::Races::Terran)
		{
			// Seek enough to one-shot an SCV.
			techScores[int(TechUnit::Mutalisks)] += 8;
		}
		else
		{
			// Seek enough to one-shot a probe or drone.
			if (nMutas < 5)
			{
				techScores[int(TechUnit::Mutalisks)] += 8;
			}
...
void StrategyBossZerg::chooseUnitMix()
{
    ...
    if (the.ops.enemySeemsDead() && hasSpire)
    {
...
// Including queens and infested terrans--units used sparingly.
void StrategyBossZerg::produceOtherStuff(int & mineralsLeft, int & gasLeft)
{
    ...
    if (_self->getUpgradeLevel(BWAPI::UpgradeTypes::Metabolic_Boost) == 0 && 
		hasPool && nDrones >= 9 && (nGas > 0 || gas >= 100) &&
    ...
    if (hasHiveTech && hasPool && nDrones >= 12 && (nGas > 0 || gas >= 200) &&
	...
	// Try not to double-tech in ZvP until we have the economy to support it.
	const bool vProtossOvertechHydras =
		the.enemyRace() == BWAPI::Races::Protoss &&
		(hasSpire || isBeingBuilt(BWAPI::UnitTypes::Zerg_Spire)) &&
		(nDrones <= 20 || nMutas + the.my.all.count(BWAPI::UnitTypes::Zerg_Scourge)/2 <= 6);
	if ((_techTarget == TechUnit::Hydralisks || _techTarget == TechUnit::Lurkers && lurkerDenTiming()) &&
        !hasDen && hasPool && nDrones >= 10 && nGas > 0 &&
		!vProtossOvertechHydras &&
        ...
        !_self->isResearching(BWAPI::TechTypes::Lurker_Aspect))
    {
        if (_self->getUpgradeLevel(BWAPI::UpgradeTypes::Muscular_Augments) == 0 &&
            !_self->isUpgrading(BWAPI::UpgradeTypes::Muscular_Augments) &&
			!_self->isUpgrading(BWAPI::UpgradeTypes::Grooved_Spines))		// could have been started by opening book
        {
	...
	// We will need lair tech in ZvT in most cases, so maybe prepare it early.
	// In ZvP it can often wait.
    // Make it anyway if the enemy already has higher tech or we have excess cash.
    if (hasPool && nLairs + nHives == 0 && nGas > 0 &&
		(lairTechUnit(_techTarget) || hiveTechUnit(_techTarget) ||
		 _enemyRace == BWAPI::Races::Terran && nDrones >= 18 && nBases >= 2 ||
         armorUps > 0 ||
         the.info.enemyHasAirTech() ||
	...
	// Try not to double-tech in PvZ until we have the economy to support it.
	const bool vProtossOvertechMutas =
		the.enemyRace() == BWAPI::Races::Protoss &&
		(hasDen || isBeingBuilt(BWAPI::UnitTypes::Zerg_Hydralisk_Den)) &&
		nDrones <= 22;
	...
	// Get it early if we already have drop, since drop is painfully slow otherwise.
	// NOTE This code doesn't research drop unless speed is already done, but the opening book may.
    if (hasLairOrHive && nGas > 0 &&
		_self->getUpgradeLevel(BWAPI::UpgradeTypes::Pneumatized_Carapace) == 0 &&
		(nDrones >= 20 || nDrones >= 15 && _self->getUpgradeLevel(BWAPI::UpgradeTypes::Ventral_Sacs) > 0) &&
        _enemyRace != BWAPI::Races::Zerg &&
        !_emergencyGroundDefense &&
        (hasSpire || hasLurkers) &&											// get your lair tech FIRST
        (_gasUnit != BWAPI::UnitTypes::Zerg_Mutalisk || nMutas >= 6) &&		// and use your lair tech first
	...
	// NOTE Disabled! Let the book do it. We can't expand by air.
    if (false &&
		hasLairOrHive && nGas > 0 && nDrones >= 18 &&
        nBases >= 3 &&
        !_emergencyGroundDefense &&
	...
	if (!hasQueensNest && hasLairTech && nGas >= 2 &&
        (nDrones >= 32 || (_recommendQueens || defilerScore > 4) && nDrones >= 20) &&
        ...
        !_self->isUpgrading(BWAPI::UpgradeTypes::Gamete_Meiosis))       // +50 energy
    {
		...
		else if (_recommendEnsnare && !_self->hasResearched(BWAPI::TechTypes::Ensnare))
        {
        ...
        else if ((_recommendEnsnare || _recommendBroodling >= 2) &&
	...
	// Require the defiler mound to be finished first. Defilers are cheaper and more important.
    ...
    {
        if (_self->getUpgradeLevel(BWAPI::UpgradeTypes::Chitinous_Plating) == 0 &&
            !_self->isUpgrading(BWAPI::UpgradeTypes::Chitinous_Plating) &&
			!_self->isUpgrading(BWAPI::UpgradeTypes::Anabolic_Synthesis))
        {
    ...
    {
		const bool secondExtractorNow =
			nDrones >= 18 ||												// ZvP
			the.enemyRace() == BWAPI::Races::Terran && nDrones >= 14 ||		// ZvT
			the.enemyRace() == BWAPI::Races::Zerg && nDrones >= 11;			// ZvZ
        ...
        else if (nGas < 2 && secondExtractorNow &&
        ...
        else if (hasLairTech && nGas < 2 && secondExtractorNow ||
	...
	// Adjust the default upgrade priorities according to the situation.
    if (_enemyRace == BWAPI::Races::Terran)
    {
	...
	// Command the upgrades, in priority order, in available evo chambers.
        ...
        !(_gasUnit == BWAPI::UnitTypes::Zerg_Mutalisk && nMutas < 6))
    {
        ...
        for (int i = 0; nFreeEvos > 0 && i < 3; ++i)
        {
			...
			if (up.isValid() && !_self->isUpgrading(up))
			{
				int upLevel = _self->getUpgradeLevel(up);

				if (upLevel < maxUpgrade())
				{
					// Tech first: Don't get upgrades > 1 unless we already have some tech.
					if (upLevel == 0 || hasDen || hasSpire || hasHiveTech)
					{
						produce(up);
						--nFreeEvos;
						mineralsLeft -= up.mineralPrice(upLevel + 1);
						gasLeft -= up.gasPrice(upLevel + 1);
					}
				}
			}
        }
	...
	// NOTE This assumes we have only one spire.
        ...
        !_self->isUpgrading(BWAPI::UpgradeTypes::Zerg_Flyer_Attacks))
    {
		...
		// Get attack first versus terran, unless there are valkyries.
		if (_enemyRace == BWAPI::Races::Terran && airAttackUps < 2 && the.your.seen.count(BWAPI::UnitTypes::Terran_Valkyrie) == 0)
		{
			getAttack = true;
		}
		// In ZvZ, get attack first versus hydras.
		else if (_enemyRace == BWAPI::Races::Zerg && airAttackUps < 2 &&
			the.your.seen.count(BWAPI::UnitTypes::Zerg_Hydralisk) > the.your.seen.count(BWAPI::UnitTypes::Zerg_Mutalisk))
		{
			getAttack = true;
		}
		// Get armor first in other cases.
		else if (airArmorUps < 2 && hasLairTech ||
            airArmorUps < 3 && hasHiveTech)
        {
			getArmor = true;
        }
		// If armor is maxed, get attack.
		...
		if (getArmor)
		{
			...
			mineralsLeft -= BWAPI::UpgradeTypes::Zerg_Flyer_Carapace.mineralPrice(airArmorUps+1);
			gasLeft -= BWAPI::UpgradeTypes::Zerg_Flyer_Carapace.gasPrice(airArmorUps+1);
		}
		else if (getAttack)
		{
			produce(BWAPI::UpgradeTypes::Zerg_Flyer_Attacks);
			mineralsLeft -= BWAPI::UpgradeTypes::Zerg_Flyer_Attacks.mineralPrice(airAttackUps+1);
			gasLeft -= BWAPI::UpgradeTypes::Zerg_Flyer_Attacks.gasPrice(airAttackUps+1);
		}
    }
	...
	if (Config::Skills::Burrow &&
		...
		nGas >= 2 &&
		(nBases >= 3 && nDrones >= 24 || (nLairs == 0 || nCompletedHatches > 1) && mineralsLeft > 350 + 75 * nLarvas && gasLeft > 300 + 50 * nLarvas) &&
		!_emergencyGroundDefense && enoughArmy())
	{
		// Burrow is researched in a hatchery, and we have more than one.
	...
	if (Config::Skills::MaxQueens > 0 &&
        ...
        nGas >= 2 &&
		// Not if we want only one queen for parasite, and the last one was recent.
		(_recommendQueens > 1 || _lastQueenOrderFrame + 120 * 24 < the.now()) &&
    ...
    {
		the.buildingPilot.land(BWAPI::UnitTypes::Zerg_Infested_Command_Center);
		// Only if at least one infested command center is on the ground.
		for (BWAPI::Unit u : the.self()->getUnits())
		{
			if (u->getType() == BWAPI::UnitTypes::Zerg_Infested_Command_Center && !u->isLifted())
			{
    ...
    // The defiler score is zero or positive, never negative.
        ...
        the.my.completed.count(BWAPI::UnitTypes::Zerg_Defiler_Mound) > 0)
    {
        ...
        else if (nDrones >= 9 && nGas > 0 && !_emergencyNow)
        {
            if (_self->hasResearched(BWAPI::TechTypes::Consume) &&
	...
	int maxDefilers = Config::Skills::MaxDefilers;
	if (the.your.seen.count(BWAPI::UnitTypes::Terran_Battlecruiser) >= 2)
	{
		maxDefilers += 2;
	}
	if (hasDefilerUps && nDefilers < maxDefilers && nGas >= 1)
    {
        int limit = std::min(maxDefilers, nGas);
        if (!_self->hasResearched(BWAPI::TechTypes::Consume) || the.ops.enemySeemsDead())
        {
            limit = 1;
        }
        if (nDefilers < limit)
...
// Produce one unit of any type, provided it can be morphed from a larva.
// Called last: The earlier work did not ask for any production. That could cause a production freeze.
// Ignore whether we're mining. If we can't produce for lack of funds, it won't hurt to have something
// in the queue anyway.
void StrategyBossZerg::produceAnything()
{
	if (nLarvas == 0 ||
		_supplyUsed >= absoluteMaxSupply - 8 ||		// very close to the supply limit
		_self->minerals() < 50)
	{
		return;										// can't produce helpfully
	}

	if (_supplyUsed >= _existingSupply + _pendingSupply)
	{
		produce(BWAPI::UnitTypes::Zerg_Overlord);
		return;
	}
	if (nDrones < 3)
	{
	...
	// What combat unit can we produce?
	// NOTE canProduce() checks extra stuff and causes issues.
	const BWAPI::UnitType types[] =
	{ _mineralUnit						// 0
	, _gasUnit							// 1
	, BWAPI::UnitTypes::Zerg_Zergling	// 2
	, BWAPI::UnitTypes::Zerg_Hydralisk	// 3
	, BWAPI::UnitTypes::Zerg_Mutalisk	// 4
	, BWAPI::UnitTypes::Zerg_Ultralisk	// 5
	};
	for (int i = 0; i <= 5; ++i)
	{
		if (types[i] == BWAPI::UnitTypes::Zerg_Hydralisk && hasDen && _enemyRace == BWAPI::Races::Protoss ||
			types[i] == BWAPI::UnitTypes::Zerg_Zergling && hasPool ||
			types[i] == BWAPI::UnitTypes::Zerg_Mutalisk && hasSpire ||
			types[i] == BWAPI::UnitTypes::Zerg_Ultralisk && hasUltra && hasUltraUps ||
			types[i] == BWAPI::UnitTypes::Zerg_Hydralisk && hasDen
			)
		{
			produce(types[i]);
			return;
	...
	// If all this fell through, we make nothing after all.
}
...
void StrategyBossZerg::handleUrgentProductionIssues(BuildOrderQueue & queue)
{
    ...
    while (nextInQueueIsUseless(queue))
    {
        ...
        if (nextInQueue == BWAPI::UnitTypes::Zerg_Hatchery)
        {
            ...
            nextInQueue == BWAPI::UnitTypes::Zerg_Spire ||
            nextInQueue == BWAPI::UnitTypes::Zerg_Hydralisk_Den && _enemyRace != BWAPI::Races::Zerg)
        {
...
// Called when the queue is empty, which implies that we are out of book.
...
BuildOrder & StrategyBossZerg::freshProductionPlan()
{
	...
	if (!WorkerManager::Instance().isCollectingGas())
	{
		if (_gasUnit != BWAPI::UnitTypes::None && gas < _gasUnit.gasPrice() ||
			gas < _mineralUnit.gasPrice())
		{
			produce(MacroCommandType::StartGas);
		}
	}
    ...
    const bool hasEnoughUnits =
        numMineralUnits + 2 * numGasUnits >= 10 &&
			(!hasSpire ||
			!the.info.enemyHasAirTech() ||
			nMutas + the.my.all.count(BWAPI::UnitTypes::Zerg_Scourge)/2 >= 7 ||
			numMineralUnits >= 6 && numGasUnits >= 6 ||
			nDrones >= 21 ||				// if we are gas-starved, we have to produceOtherStuff() to get gas
			nLings >= 20					// ditto
			);
    ...
    if (hasEnoughUnits)
    {
        produceOtherStuff(mineralsLeft, gasLeft);
	...
	// Prevent production freezes.
	// If the build order is empty, produce something--anything--if possible.
	if (_latestBuildOrder.size() == 0)
	{
		produceAnything();
	}
