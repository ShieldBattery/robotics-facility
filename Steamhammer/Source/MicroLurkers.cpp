    ...
    for (const auto & kv : the.info.getUnitData(the.enemy()).getUnits())
    {
        ...
        if (!ui.goneFromLastPosition && !ui.lifted && ui.unit)
        {
            if (ui.unit->isVisible())
            {
                ...
                BWAPI::WeaponType enemyWeapon = UnitUtil::GetGroundWeapon(ui.type);
                if (enemyWeapon.damageAmount() >= 20 ||
					ui.type == BWAPI::UnitTypes::Terran_Bunker ||
					ui.type == BWAPI::UnitTypes::Protoss_Carrier)
                {
                    int enemyRange = UnitUtil::GetYourAttackRange(ui.type, BWAPI::UnitTypes::Zerg_Lurker);
                    if (lurker->getDistance(ui.lastPosition) <= enemyRange + 31)
                    {
...
bool MicroLurkers::safeToBurrow(const BWAPI::Unitset & lurkers, BWAPI::Unit lurker) const
{
    return
        lurkers.size() >= 4 ||
        the.groundHitsFixed.at(lurker->getTilePosition()) == 0 ||
...
void MicroLurkers::executeMicro(const BWAPI::Unitset & targets, const UnitCluster & cluster)
{
    ...
    for (BWAPI::Unit lurker : lurkers)
    {
        ...
        if (target)
        {
			...
			// Close enough to burrow.
            if (dist <= 66 || isThreat && dist <= lurkerRange)
            {
