...
private:
	...
	void cancelAtDoomedBases();					// STEP 0
	...
	bool SCVinDanger(const Building & b) const;
	bool safeToKeepConstructing(const Building & b) const;
	bool buildingInDanger(const Building & b) const;
	...
	void liftBlockingInfestedCC(Building & b);
...
public:
	...
	size_t              getNumUnstarted(BWAPI::UnitType type, const Base * base) const;
	...
	void				cancelBuildingsAtDoomedBase(Base * base);
