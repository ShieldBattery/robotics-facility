...
void ProductionManager::update() 
{
	...
	//      a worker inside the refinery has order HarvestingGas and no location
	...
	// NOTE This may insert items into the queue!
...
void ProductionManager::onUnitDestroy(BWAPI::Unit unit)
{
    ...
    if (unit->getType().isWorker() && !_outOfBook)
    {
        ...
        else if (the.selfRace() == BWAPI::Races::Terran)
        {
            ...
            if (nRax > 0 &&
                the.my.all.count(BWAPI::UnitTypes::Terran_Marine) < nRax &&
        ...
        else // zerg
        {
            ...
            {
                goOutOfBookAndClearQueue();      // too many drones lost, go out of book anyway
            }
...
bool ProductionManager::itemCanBeProduced(const MacroAct & act) const
{
	...
	if (act.isCommand() || act.isAddon() || act.isUpgrade() || act.isTech())
...
BWAPI::Unit ProductionManager::getProducer(MacroAct act) const
{
	...
	std::vector<BWAPI::Unit> candidateProducers = act.getCandidateProducers();
...
void ProductionManager::executeCommand(const MacroAct & act)
{
    ...
    {
        the.opsStrategy.setAggression(AggressionLevel::Aggressive);
    }
    else if (cmd == MacroCommandType::Defensive)
    {
		the.opsStrategy.setAggression(AggressionLevel::Defensive);
    }
    ...
    {
        the.buildingPilot.lift(act.getCommandType().getUnitType());
    }
...
// We already have a production goal for the given unit type.
// This is used to prevent accidental multiple addons.
bool ProductionManager::alreadyHasGoal(BWAPI::UnitType unitType) const
{
	for (const ProductionGoal & goal : _goals)
	{
		if (goal.act.isUnit() && goal.act.getUnitType() == unitType)
		{
			return true;
		}
	}
...
// We have finished our book line, or are breaking out of it early, or
// are clearing the queue due to some other emergency.
...
void ProductionManager::goOutOfBookAndClearQueue()
{
	if (Config::Debug::DrawQueueFixInfo && !_queue.isEmpty())
	{
		if (_outOfBook)
		{
			BWAPI::Broodwar->printf("queue: clear production queue");
		}
		else
		{
	...
	if (!_outOfBook)
	{
		if (the.evaluator.canEvaluate())
		{
			double score = the.evaluator.evaluate();
			the.oppModel().setOpeningScore(score);

			//BWAPI::Broodwar->printf("queue: going out of book, eval = %1.3f", score);
		}
		else
		{
			// The score will default to -1.0, meaning we don't have one.
			//BWAPI::Broodwar->printf("queue: going out of book, no evaluation");
		}
	}
