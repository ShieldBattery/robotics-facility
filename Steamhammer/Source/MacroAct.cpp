    ...
    , _parent(nullptr)
{
	...
	MacroLocation specifiedMacroLocation(MacroLocation::Anywhere);    // the default
	{
		...
		std::smatch match;
		if (std::regex_match(inputName, match, macroLocationRegex))
		{
			std::string nameCopy = match[1].str();
			specifiedMacroLocation = getMacroLocationFromString(match[2].str());
			inputName = nameCopy;
		}
	}
    ...
    if (inputName.substr(0, 3) == std::string("go "))
    {
        for (MacroCommandType t : MacroCommand::allCommandTypes())
        {
            ...
            if (MacroCommand::hasNumericArgument(t))
            {
                ...
                if (std::regex_match(inputName, nameMatch, commandWithArgRegex))
                {
                    int amount = GetIntFromString(nameMatch[1].str());
                    if (amount >= 0)
                    {
            ...
            else if (MacroCommand::hasUnitArgument(t))
            {
                ...
                if (std::regex_match(inputName, nameMatch, commandWithArgRegex))
                {
                    BWAPI::UnitType unitType = getUnitTypeFromString(nameMatch[1].str());
...
// to make one of them.
// NOTE To make an archon or dark archon requires zero more supply, but they are not made through
//      the production system so they're not included here.
...
int MacroAct::mineralPrice() const
{
    ...
    if (isUpgrade())
    {
		// Price to upgrade one more level. We assume without checking that it's not already maxed.
	...
	if (isCommand()) {
		if (_macroCommandType.getType() == MacroCommandType::ExtractorTrickDrone ||
			...
			// this is what you need to perform it.
			return 100;
		}
		return 0;
	}
...
bool MacroAct::isProducer(BWAPI::Unit unit) const
{
	...
	if (unit->getOrder() == BWAPI::Orders::LiftingOff)
								{ return false; }
    if (!unit->isPowered())		{ return false; }
	// There's latency between ordering a unit and isTraining() becoming true.
	// This test prevents more than one unit from being queued up for production.
	if (unit->isTraining() || 
		unit->getLastCommand().getType() == BWAPI::UnitCommandTypes::Train &&
    ...
    if (isUnit())
    {
        ...
        for (const ReqPair & pair : getUnitType().requiredUnits())
        {
			...
			{
				if (requiredType == BWAPI::UnitTypes::Terran_Machine_Shop ||
					requiredType == BWAPI::UnitTypes::Terran_Control_Tower)
				{
					if (!unit->getAddon())
					{
						// No need to check that the addon is the required type, only one type is possible.
						return false;
					}
				}
				else if (requiredType == BWAPI::UnitTypes::Terran_Physics_Lab ||
					requiredType == BWAPI::UnitTypes::Terran_Covert_Ops)
				{
		...
		// NOTE This is not needed because we handle it at a higher level.
		// Check for required research. Need Lurker Aspect to make a lurker (that's the only one).
		//if (!the.self()->hasResearched(getUnitType().requiredTech()))
		//{
		//	return false;
		//}
    }
...
std::vector<BWAPI::Unit>  MacroAct::getCandidateProducers() const
{
	std::vector<BWAPI::Unit> candidates;

	if (isCommand())
    {
        UAB_MESSAGE("no producer of a command");
        return candidates;
    }
...
// or a producer is being created.
bool MacroAct::hasEventualProducer() const
{
    ...
    for (BWAPI::Unit unit : the.self()->getUnits())
    {
        ...
        if (unit->getType() == producerType &&
			...
			// If we want to produce an addon, the producer must not already have one.
			(!isUnit() || !getUnitType().isAddon() || !unit->getAddon()))
		{
            return true;
        }
	...
	// Maybe a producer is under construction.
	return
		the.my.all.count(producerType) > the.my.completed.count(producerType) ||
		BuildingManager::Instance().getNumUnstarted(producerType) > 0;
}
...
bool MacroAct::hasPotentialProducer() const
{
    ...
    for (BWAPI::Unit unit : the.self()->getUnits())
    {
        ...
        if (unit->getType() == producerType &&
            ...
            !unit->isResearching() &&
			// If we want to produce an addon, the producer must not already have one.
			(!isUnit() || !getUnitType().isAddon() || unit->canBuildAddon()))
		{
            return true;
        }
...
bool MacroAct::hasTech() const
{
    ...
    // which matters only for merging archons and dark archons (which is not done via MacroAct).
...
bool MacroAct::canProduce(BWAPI::Unit producer) const
{
    if (isCommand())
    {
        // NOTE Not strictly correct for an extractor trick (it may execute but do nothing).
        return true;
    }
...
{
    UAB_ASSERT(producer, "producer was null");
...
void MacroAct::cancelUnfinished() const
{
	// Commands cannot be canceled.

	if (isUnit())
	{
		for (BWAPI::Unit u : the.self()->getUnits())
		{
			if (!u->isCompleted() && u->getType() == getUnitType())
			{
	...
	{
		if (the.self()->isUpgrading(getUpgradeType()))
		{
			for (BWAPI::Unit u : the.self()->getUnits())
			{
				if (u->isUpgrading() && u->getUpgrade() == getUpgradeType())
				{
					u->cancelUpgrade();
				}
			}
	...
	{
		if (the.self()->isResearching(getTechType()))
		{
			for (BWAPI::Unit u : the.self()->getUnits())
			{
				if (u->isResearching() && u->getTech() == getTechType())
				{
					u->cancelResearch();
				}
			}
