...
BWAPI::Unit MicroMelee::getTarget(BWAPI::Unit meleeUnit, const BWAPI::Unitset & targets, bool underThreat)
{
    ...
    for (const auto target : targets)
    {
        ...
        if (!underThreat)
        {
            ...
            // Also prefer to attack stuff that can't shoot back: Remain out of threat.
		...
		// Dark templar get a big bonus for attacking a target not in detection range.
		// (Don't make the bonus so big that it runs past a cannon.)
		if (meleeUnit->getType() == BWAPI::UnitTypes::Protoss_Dark_Templar && !UnitUtil::EnemyDetectorInRange(target->getPosition()))
		{
			score += 5 * 32;
		}
...
int MicroMelee::getAttackPriority(BWAPI::Unit attacker, BWAPI::Unit target) const
{
    ...
    if (attacker->getType() == BWAPI::UnitTypes::Protoss_Dark_Templar)
    {
        ...
        if (targetType == BWAPI::UnitTypes::Terran_Missile_Turret ||
			targetType == BWAPI::UnitTypes::Terran_Comsat_Station && target->isCompleted())
        {
            return 9;
        }
...
bool MicroMelee::meleeUnitShouldRetreat(BWAPI::Unit meleeUnit, const BWAPI::Unitset & targets)
{
	...
	// An undetected dark templar should not retreat.
	if (meleeUnit->getType() == BWAPI::UnitTypes::Protoss_Dark_Templar && !UnitUtil::EnemyDetectorInRange(meleeUnit))
	{
		return false;
	}
